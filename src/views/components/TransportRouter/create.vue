<template>
  <div class="container create-transport">
    <div class="create-transport__header">
      <h3>THÊM MỚI TRANSPORT ROUTE</h3>
    </div>
    <div class="create-transport__content">
      <div class="row">
        <div class="col-md-12">
          <BaseInputCustom
              ref="name"
      ...
      <div class="row">
        <div class="col-md-12">
          <BaseInputCustom
              ...
              type="text"
              error-required="Trường English Translation Phone không được để trống!"
          />
        </div>
      </div>
      <div class="action">
        ...
        <button
            ...
            @click.prevent="createTransportRoute"
        >
          <span class="btn-inner--icon">
  ...
  methods: {
    async createTransportRoute () {
      try {
        ...
        if (refsValid) {
          const res = await transportService.createTransport(this.formData)
          await Swal.fire({
            title: `Một Transport Route mới được tạo với định danh ${res.id}`,
            text: ``,
            icon: 'success',
