<template>
  <div class="container update-transport">
    <div class="update-transport__header">
      <h3>CẬP NHẬT USER PROFILE {{ formData.id }}</h3>
    </div>
    <div class="update-transport__content">
      <div class="row">
        <div class="col-md-12">
          <BaseInputCustom
              ref="id"
      ...
      <div class="row">
        <div class="col-md-12">
          <BaseInputCustom
              ...
              label="Name"
              type="text"
              error-required="Trường Name không được để trống!"
          />
        </div>
      </div>
      <div class="row">
        <div class="col-md-12">
          <BaseInputCustom
              ...
              v-model="formData.englishTranslation"
              class-name="theme-light"
              name="englishTranslation"
              label="English Translation"
              type="text"
              error-required="Trường English Translation không được để trống!"
          />
        </div>
      </div>
      <div class="action">
...
export default {
  ...
  methods: {
    async getDetail () {
      ...
      try {
        const res = await transportService.detailTransport(id)
        this.formData.id = res.id
        this.formData.name = res.name
        this.formData.englishTranslation = res.englishTranslation
      }catch (e) {
        console.log(e)
      }
    },
    async updateTransport() {
      try {
        ...
        if (refsValid) {
          const res = await transportService.updateTransport(this.formData)
          await Swal.fire({
            title: `Cập nhật transport route thành công với định danh là ${res.id}`,
            text: ``,
            icon: 'success',
            ...
            if (result.isConfirmed) {
              await this.$router.push('/transport-route')
            }
          });
        }
      } catch (e) {
